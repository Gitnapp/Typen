...
- 保留关闭前的未保存内容确认流程。
- 新增双窗口回归测试，并修正 macOS 测试宿主的应用路径。

## [2026-08-14] v0.5.0 —— 编辑器设置重做 + 控件风格统一

### 列宽 → 缩进

「列宽」原本设定的是阅读栏的**总宽度**，窗口拉宽时文字区被锁死在那个宽度里。现在换成「缩进」——直接设定文字距左右两边的边距（24/40/64/96/140/200/280，七档），窗口变宽时文字区跟着变宽，边距保持不变。窄窗口下有保护：缩进不会把文字区挤到 240px 以下。

### 新增「自动换行」开关（默认开）

关掉后源码区进入横向滚动，长行不再折行，按最长行取宽。

### 「字号」→「基础字号」，改成分档

改名是因为它本来就不只管正文——标题、行内代码都是它的倍数，动它等于动整份文档的排版比例。改成 12/13/14/15/16/18/20/22/24 九档，拖动吸附刻度、轨道画刻度点。旧配置里的非整档值（如 15.3）读取时自动吸附到最近一档。

### 控件风格统一

- **无外框**：分段控件、对话框次要按钮、还原默认、检查更新、快捷键输入框全部去掉边框，只用填充色区分状态。
- **hover 高亮**：填充按钮 hover 时沿 surface 色阶上升一档。这里有个坑：原本给填充按钮用的是半透明白色遮罩，浅色模式下白盖浅色几乎看不见，所以 surface 填充的按钮改用不透明的 `surface3`。
- **尺寸统一**：新增 `kControlHeight = 28`，所有按钮类控件共用。之前是 24/26/26/34/38 五种高度。Material 按钮还需额外设 `minimumSize` + `shrinkWrap`，否则它强制的 48px 最小点击区会把统一好的高度顶开。
- **行高统一**：设置页每行固定 42px。之前用垂直 padding，行高由各自控件决定——slider 天生矮、开关和 chips 天生高，三种行高各不相同。
- **不再用蓝色**：`accent` 从固定的 systemBlue 改成派生自 `textPrimary`——深色模式是白色，浅色模式自动反成近黑；配套的 `onAccent` 给填充上的文字用。写死白色的话浅色模式下白底白卡会看不见，所以实现成"对比度最高的那个颜色"。

### 验证

- `flutter analyze` 0 issue，`flutter test` 100 个测试全过（新增 8 个：缩进分档、换行默认值与横向滚动、字号吸附、控件更名、无外框、行高统一、控件不被行高撑开、尺寸统一）
- 其中「控件不被行高撑开」和「尺寸统一」两条都是先写测试抓出了真实问题：前者抓到快捷键框被 42px 行高撑成色块，后者抓到我把侧边栏项和品牌图标误算进控件集合

## [2026-08-14] v0.4.3 —— 切到预览时内容不再水平居中（#6）

### 根因

`Scaffold` 的 `Column` 用的是默认的 `crossAxisAlignment: center`，编辑器面板拿到的是**松约束**，于是会按内容宽度收缩、并被水平居中。

源码模式看不出来——`TextField` 会把给它的宽度全部占满，所以面板始终是满宽的。切到预览就露馅了：`MarkdownBody` 按内容宽度收缩（一句「123」只有几十像素宽），整个面板跟着缩成一条，再被 `Column` 居中——看起来就是"切到预览，文字自己跑到中间去了"。

这也是 issue #6 的真正原因。之前几轮（包括 issue 里的分析）都往"滚动比例映射"方向查了，因为现象被描述成"自动居中"时，很自然会理解成垂直方向的滚动位置；直到这次的两张对比截图才看清是**水平**方向。

### 修法

- `Column` 改成 `crossAxisAlignment: stretch`，面板拿到紧约束。
- 预览再用 `Positioned.fill` 铺满 `Stack`。只改前一条的话文字位置就正确了，但 `SingleChildScrollView` 仍按内容收缩，滚动条会贴在内容右侧而不是窗口边缘——那是这段代码原本就写明的设计意图。

### 验证

新增回归测试：同一份文档在源码和预览下，文字左边缘必须落在同一处。修复前预览是 440.0（在 800 宽的窗口里正好居中），源码是 40.0；修复后两边都是 40.0。已验证"改前红、改后绿"。

- `flutter analyze` 0 issue，`flutter test` 91 个测试全过

## [2026-08-14] v0.4.2 —— 标题栏源码/预览按钮收紧

标题栏里那个「源码 / 预览」小按钮的内边距偏大，两个字被撑得松垮，和旁边的文件名不协调。内边距从 9×3 收到 7×2。

### 验证

- `flutter analyze` 0 issue，`flutter test` 90 个测试全过

## [2026-08-14] v0.4.1 —— 设置页控件右对齐 + 光标统一

### 控件右对齐

设置页每一行的控件（快捷键框、分段控件、滑块、关于页的 GitHub 链接）现在都贴着右边缘，一列对齐，而不是各自跟在标签后面从不同位置开始。快捷键行的「已修改」标注挪到了框的左侧，好让框本身保持在最右。

### 设置窗口不再出现手型光标

分段控件用的是 `SystemMouseCursors.click`（手型），滑块则是 Flutter `Slider` 自己的默认手型——设置窗口里的控件应该读作原生 macOS chrome，手型在 macOS 语境里意味着链接。全部改成默认箭头。

写的回归测试直接抓出了滑块这个我漏掉的地方：断言遍历设置窗口里所有 `MouseRegion`，任何一个用手型都会红。

### 验证

- `flutter analyze` 0 issue，`flutter test` 90 个测试全过（新增 1 个右对齐 + 光标的回归测试）

## [2026-08-14] v0.4.0 —— 快捷键自定义

设置窗口新增「快捷键」分类，可以重新绑定 Typen 自己的命令，并且能一键还原默认。

### 可自定义的命令

新建、新建窗口、打开、关闭窗口、保存、另存为、切换源码/预览、查找、查找与替换、查找下一个、查找上一个、偏好设置——共 12 条。

**标准编辑键（撤销/重做/剪切/拷贝/粘贴/全选）刻意不做自定义**：它们是系统级约定，而且菜单项并不是唯一绑定——Flutter 的文本框自己也响应这些键，改了菜单会出现"旧键还能用"的诡异现象。

### 交互

点一下某行的快捷键框进入录制，按下新组合键即生效，按 Esc 取消。已经被别的命令占用的组合键会被拒绝，并直接显示「已被「XX」占用」，而不是让两个命令抢同一个键。被改过的行会标注「已修改」。

「重置」区块的**还原默认**按钮一次性清掉所有自定义；全都是默认值时按钮自动禁用。

### 实现要点

- 只存**与默认不同**的绑定。这样以后某个命令的默认键变了，没手动改过它的用户会跟着新默认走。
- 按键存的是 `keyId` 而不是标签——id 跨版本和键盘布局都稳定，标签不是。
- 存储损坏不会崩：解析失败就整体回落到默认值。
- 菜单栏改成从 Settings 读取绑定，配合已有的跨窗口 `settingsChanged` 广播，在设置窗口改完，已经开着的编辑器窗口菜单会跟着更新。

### 顺带修掉的布局问题

设置页内容原来是**垂直居中**的——`Scaffold` 里那个 `Row` 用了默认的 `crossAxisAlignment: center`，页面比窗口矮时会缩成一团停在中间，标题上方空出一大片。改成 `stretch`，内容现在从顶部开始排。加了回归测试锁住（改之前标题距顶 141.5px，测试会红）。

### 验证

- `flutter analyze` 0 issue，`flutter test` 89 个测试全过（新增 8 个快捷键测试 + 1 个顶部对齐回归测试）
- 顶部对齐那条测试已验证"改前红、改后绿"

## [2026-08-14] v0.3.15 —— 确认弹窗对齐设置页卡片风格（#9-1）

处理 issue #9 的三条，只有第一条查证清楚并修好了，另外两条的结论写在 issue 评论里。

### 确认弹窗改用和设置页一致的卡片 chrome

「有未保存的修改」这类确认弹窗原来是 `surface1` + 无边框 + `elevation: 24` 的阴影卡片，而设置页的卡片是 `surface0` + 一像素边框 + 无阴影——同一套"圆角卡片"用了两套 chrome，弹窗明显更"重"。现在弹窗收敛到设置页那一套：`surface0` + `Border.all(p.border)` + `elevation: 0`，圆角仍是共用的 `kRadiusSurface`。

### 顺带新增一个不变量测试

单行（不足一屏）的文档切到预览必须顶对齐——这条是为了排查 #9 的第三点写的。测试在当前代码上**通过**，也就是说那个现象在组件测试里复现不出来，但这个不变量值得钉住，防止以后真的引入居中行为。

### 验证

- `flutter analyze` 0 issue，`flutter test` 80 个测试全过

## [2026-08-13] v0.3.14 —— 修复切换 app 后窗口失去响应（已复现并验证）

这次是真的定位到根因、复现、并验证修好了。

### 根因

切走 Typen 时，框架收到 `AppLifecycleState.hidden`；**切回来时没有任何生命周期事件**——`applicationDidBecomeActive` 根本不会到达本应用的 delegate（加日志实测：整个过程调用 0 次，所以 v0.3.13 里补的那个 `super` 调用其实是死代码，也解释了它为什么没效果）。

于是每个引擎都永远停在 `hidden` 状态。关键在于：**按键和鼠标事件其实照样送达 Dart**（这点也实测确认了），只是框架认为 app 还处于隐藏状态，焦点系统就忽略它们——表现出来就是"窗口还在画、但完全不响应"。生命周期是 app 级的，这也解释了为什么会牵连到其他窗口。

### 修法

`becomeKey` 是切回来时**确实会触发**的信号，用它把 `AppLifecycleState.resumed` 通过 `flutter/lifecycle` 通道推给该窗口自己的引擎（每个窗口一个引擎，所以要逐个推）。`AppDelegate` 另外监听 app 级的 `didBecomeActive`/`didUnhide` 通知作为兜底。

### 复现与验证

之前几个版本反复改错，是因为我的探针测的是**渲染**——而引擎冻结时照样出帧、照样能重新布局，所以一律误报"正常"。换成**键盘输入探针**（Tab 聚焦 + 方向键推滑块，看数值变不变）后才复现出来：

- 不切换 app：4772 像素变化（正常）
- `activate` / ⌘Tab / ⌘H 三种切换：**全部 0 像素变化**（失去响应）
- 打上本次修复后：滑块被方向键从 760 推到 1100，**输入完全恢复**

日志也确认生命周期跑通了：`hidden → inactive → resumed`。
...
之前 v0.3.10 加的 `animationDuration: Duration.zero` 只作用在对话框的 Material 按钮上，跟侧边栏这个自己手写的 hover 完全没关系，所以那次改动对这个现象无效。

顺带修正了这套测试的一个盲点：原来断言读的是 `Container.decoration`（**目标值**），哪怕动画正跑到一半也会读到最终颜色，所以"有没有渐变"这件事以前根本测不出来。现在改成读实际绘制的 `DecoratedBox`，并新增一个测试：hover 之后只 `pump()` 一帧就必须已经是最终颜色。这个测试在改动前会失败（一帧后仍是完全透明，说明确实在渐变中），改动后通过。

### 验证

- `flutter analyze` 0 issue，`flutter test` 77 个测试全过
- 新增测试已验证"改前红、改后绿"，不是空过的测试

## [2026-08-13] v0.3.11 —— 侧边栏 hover 真正的闪烁根因 + 恢复检查更新按钮

v0.3.10 的 `animationDuration: Duration.zero` 是修错了地方——那个改动只影响对话框里的 Material 按钮，用户反馈的闪烁其实发生在偏好设置左侧边栏（外观/关于）。

### 侧边栏 hover 闪烁的真正根因

`_SidebarItem` 每一项之间只有 1.5px 的垂直间距，而且这段 padding 一直是包在 `MouseRegion` **外面**的——也就是说相邻两项之间有大约 3px 的死区，两边的 `MouseRegion` 都不覆盖。鼠标只要有一点点抖动（哪怕手感觉不动，真实的鼠标/触控板传感器噪声也经常有好几个像素），只要落在这个死区里，hover 就会先关掉、再等鼠标移回去才重新点亮——看起来就是"浅-深-浅"来回闪。修法是把 `MouseRegion` 挪到最外层，包住这段 padding，死区从此不存在。

写了个组件测试专门验证这个：模拟鼠标在同一位置附近做亚像素抖动 40 次，颜色不能变回透明；再测一次"悬停在行的旧死区边缘"，颜色也必须还是 hover 色——这两个测试改之前会挂，改之后都过。

### 恢复"检查更新…"按钮

v0.3.9 去掉过一次，这次按反馈加回来了。

### 验证

- `flutter analyze` 0 issue，`flutter test` 76 个测试全过（新增 2 个专门针对这次 hover 死区问题的回归测试）
